...
### 満席通知機能（v2.4）
- **自動満席検知**：30秒間隔で満席状態を自動検知
- **リアルタイム通知**：満席時に画面上に通知バナーを表示
- **Web Push通知**：ブラウザのプッシュ通知機能を使用した満席通知
- **管理者向け通知**：管理者画面での満席状態の可視化
- **通知履歴**：過去の満席通知の履歴表示
...
- **`offline-sync-v2.css`**: オフライン同期UI
  ...
  - 依存: `styles.css`
- **`sw.js`**: Service Worker（v2.4 満席通知対応版）
  - 段階的キャッシュ（クリティカル6個→セカンダリ20個）
  - メモリ圧迫防止（バッチサイズ3個、100ms待機）
  - iOS対応最適化、PWA更新検知機能、Web Push通知処理
  - 依存: なし（独立）
- **`push-notification.js`**: Web Push通知システム
  - 満席通知のサブスクリプション管理
  - VAPID公開鍵取得、通知許可管理
  - Service Workerとの連携
  - 依存: `config.js`
...
| **push-notification.js** | 318 | Web Push通知機能 |
...
### 概要
満席通知機能は、特定の時間帯が満席になった際に、管理者やユーザーにリアルタイムで通知するシステムです。Web Push API を活用し、ブラウザのプッシュ通知機能と連携して、アプリケーションが閉じられていても通知を受け取ることができます。
...
sequenceDiagram
    ...
    participant Server as サーバー(GAS)
    
    Client->>Server: 満席状態チェック (30秒間隔)
    Server-->>Client: 満席情報を返却
    
    alt 新たに満席になった時間帯がある場合
        Client->>SW: 満席通知イベント送信
        SW-->>Client: 画面上に通知バナー表示
        Client->>Server: プッシュ通知送信リクエスト
        Server-->>SW: プッシュ通知送信
        SW-->>Client: ブラウザ通知表示
    end
```
...
### 設定方法
1. **VAPID キーの設定**
   - `push-notification.js` の `vapidPublicKey` を適切な値に設定
   - GAS 側でも対応する秘密キーを設定

2. **通知権限の取得**
   - 初回利用時にブラウザの通知許可を取得
   - 「通知を許可」を選択することで機能が有効化

3. **通知頻度の調整**
   - `logs-main.js` の `checkFullTimeslotsAndNotify` 関数内の間隔を調整
   - デフォルトは30秒間隔

### 使用方法
- **管理者画面**: 自動的に満席状態を監視し、新たに満席になった時間帯があれば通知
- **通知の確認**: 画面上のバナーまたはブラウザの通知センターで確認可能
- **通知のクリック**: 通知をクリックすると該当の時間帯の座席ページに移動
